...
#[no_mangle]
#[no_mangle]
fn detect_data_type(s: *mut internal_state) -> i32 {
    let mut block_mask: uLong = 0xf3ffc07f;
    let mut n: i32;
    for n in 0..=31 {
        if (block_mask & 1) != 0 && (*s).dyn_ltree[n as usize].fc.freq != 0 {
            return 0;
        }
        block_mask >>= 1;
    }
    ...
    for n in 32..256 {
        if (*s).dyn_ltree[n as usize].fc.freq != 0 {
            return 1;
        }
    }
    return 0;
